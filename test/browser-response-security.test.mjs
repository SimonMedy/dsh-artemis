...
test('plugin trust-fence rejections inherit the browser security baseline on every route', async () => {
  const registrations = []
  let dispose
  const ctx = {
    webServer: {
  ...
  await serve((req, res) => {
    ...
    for (const route of registrations) {
      ...
      assert.equal(response.headers.get('cross-origin-resource-policy'), 'same-origin', `${route.path} permits cross-origin embedding`)
      assert.equal(response.headers.get('referrer-policy'), 'no-referrer', `${route.path} permits referrer disclosure`)
