window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region src/shared/content-length.mjs
		function parseDecimalContentLength(value) {
			if (value === null || value === void 0) return null;
			if (typeof value !== "string" || !/^\d+$/.test(value)) throw new TypeError("Content-Length must be a decimal integer");
			const parsed = Number(value);
			if (!Number.isSafeInteger(parsed)) throw new TypeError("Content-Length must be a safe integer");
			return parsed;
		}
		//#endregion
		//#region src/shared/json-content-type.mjs
		const APPLICATION_JSON_PATTERN = /^application\/(?:json|[!#$%&'*+\-.^_`|~0-9a-z]+\+json)$/i;
		function jsonMimeType(value) {
		...
		//#endregion
		//#region src/client/response-body-cleanup.mjs
		...
		function assertJsonContentType(response, label) {
			if (!isJsonContentType(responseHeader(response, "content-type").toLowerCase())) throw new Error(`${label} returned an invalid content type`);
		}
		function assertDeclaredLength(response, maxBytes, label) {
			...
			if (length > maxBytes) throw new Error(`${label} exceeded the response size limit`);
		}
		...
		async function readBoundedJsonResponse(response, { maxBytes = DEFAULT_BROWSER_JSON_LIMIT_BYTES, label = "JSON response" } = {}) {
			...
			try {
				while (true) {
					...
					if (total > maxBytes) throw new Error(`${label} exceeded the response size limit`);
					chunks.push(value);
				}
			} catch (error) {
				try {
		...
		//#endregion
		//#region src/shared/timer-delay.mjs
		...
		async function readResponseBytes(response, maxBytes) {
			...
			try {
				while (true) {
					...
					if (size > maxBytes) throw new Error("Android snapshot exceeded the browser size limit");
					chunks.push(value);
				}
			} catch (error) {
				try {
