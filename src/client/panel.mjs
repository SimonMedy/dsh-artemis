...
function DeviceCard({ overview }) {
  ...
  return h('section', { style: styles.card, 'aria-label': 'Android device' },
    ...
    h('div', { style: styles.facts },
      h('span', null, 'ADB state'),
      h('span', { style: styles.factValue }, device.state),
      h('span', null, 'Product'),
      h('span', { style: styles.factValue }, device.product ?? '—'),
      h('span', null, 'Screen stream'),
      h('span', { style: styles.factValue }, overview.stream.connected ? 'Connected' : 'Idle'),
    ),
  )
}
