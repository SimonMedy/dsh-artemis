...
7. removes `dsh-artemis` through the same public `dsh plugin` interface, then verifies both the profile dependency and `dsh.profile.bundles` entry are gone and that the base Harness profile still composes without Artemis;
8. reinstalls the same packed tarball and verifies dependency, Cordis layer and composed config are restored;
9. boots authenticated DeepSeek Harness Web from that reinstalled state and runs the real Chromium journey.
...
The install/remove/reinstall cycle is intentionally exercised only through public Harness plugin/profile interfaces; `dsh-artemis` never patches the Harness checkout or mutates its source tree. DeepSeek Harness is configured only through public Host APIs. The browser test creates a real Workspace and Session through public RPC, submits one short prompt to make the Session non-blank, reloads through the native Workspace lifecycle, opens Android through the shipped right-sidebar entry, then verifies the panel, setup/status, device metadata, task/trace evidence, screenshot/live behavior and visual QA flows.
